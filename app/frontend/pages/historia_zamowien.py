...
def send_review(username, order_id, product, review):
    ...
    try:
        response = requests.post("http://history_orders:8007/review", json=review_data)
        response.raise_for_status()
    ...
    if orders:
        for order in orders:
            if st.button(f"Zamówienie #{order['order_id']}"):
                st.session_state[f"expanded_order_{order['order_id']}"] = not st.session_state.get(f"expanded_order_{order['order_id']}", False)
